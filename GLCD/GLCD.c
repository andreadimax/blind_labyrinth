...
* Description    : Draw the following box on LCD Screen:
*                 
*              x0 y0                            x1 y1
*               -----------------------------------
*               |                                 |
...
void LCD_DrawBox( uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1 ,uint16_t x2, uint16_t y2 , uint16_t x3, uint16_t y3,  uint16_t color )
{
	uint8_t x_value;
	
	//Check values
	if((x0 != x2 | x1 != x3 | y0 != y1 | y2 != y3) | (x1< x0 | x3 < x2 | y2 < y0 | y3 < y1))
		return;
	
	//Draw Box
	for(x_value = x0; x_value <= x1; x_value++){
		LCD_DrawLine(x_value,y0,x_value,y2, color);
	}
	return;
}
